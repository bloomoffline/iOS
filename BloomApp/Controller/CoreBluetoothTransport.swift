...
fileprivate let log = Logger(label: "BloomApp.CoreBluetoothTransport")
...
class CoreBluetoothTransport: NSObject, ChatTransport, CBPeripheralManagerDelegate, CBCentralManagerDelegate, CBPeripheralDelegate {
    ...
    private class DiscoveredPeripheral: CustomStringConvertible {
        ...
        var description: String { "DiscoveredPeripheral (isBloom: \(isBloom), userID: \(userID.map(\.uuidString) ?? "?"), userName: \(userName ?? "?")" }
    ...
    private func updateNetwork() {
        ...
        DispatchQueue.main.async { [self] in
            network.nearbyUsers = peripherals.filter(\.value.isConnected).filter(\.value.isBloom).map { (peripheral: CBPeripheral, discovered) in
                NearbyUser(
    ...
    private func publishService() {
        log.info("Publishing Bloom GATT service...")
    ...
    private func startAdvertising() {
        ...
        peripheralManager.startAdvertising([
            ...
            CBAdvertisementDataLocalNameKey: "Bloom"
        ])
    }
    ...
    func peripheral(_ peripheral: CBPeripheral, didDiscoverServices error: Error?) {
        ...
        if let service = peripheral.services?.first(where: { $0.uuid == serviceUUID }) {
            log.info("Found our Bloom service on the remote peripheral, looking for characteristic...")
    ...
    func peripheral(_ peripheral: CBPeripheral, didDiscoverCharacteristicsFor service: CBService, error: Error?) {
        ...
        if service.uuid == serviceUUID, let characteristics = service.characteristics {
            log.info("Found Bloom service on remote peripheral \(peripheral.name ?? "?") with \(characteristics.count) characteristics.")
