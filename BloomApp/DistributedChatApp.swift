...
private let log = Logger(label: "BloomApp.BloomApp")
...
struct BloomApp: App {
    ...
    var body: some Scene {
        WindowGroup {
            ContentView(controller: state.controller)
                        ...
                        center.requestAuthorization(options: [.alert, .sound, .badge]) { granted, error in
                            ...
                            if granted {
                                state.controller.onAddChatMessage { message in
                                    if !state.settings.notification.enable {
                                        return
                                    }
                                    ...
                                    content.targetContentIdentifier = "bloomoffline:///message/\(message.id)"
                                    content.sound = UNNotificationSound.default       
                                    let request = UNNotificationRequest(identifier: "Bloom message", content: content, trigger: nil)
                ...
                .onOpenURL { url in
                    guard url.isBloomSchemed else { return }
                    
                    // bloomoffline:///channel           --> channel #global
                    // bloomoffline:///channel/room:test --> channel #test
                    // bloomoffline:///message/<uuid>    --> specific message (TODO: Actually scroll to message)
    ...
    init() {
        ...
        UIView.appearance(whenContainedInInstancesOf: [UIAlertController.self]).tintColor = UIColor(named: "primary_green")
    }
}
